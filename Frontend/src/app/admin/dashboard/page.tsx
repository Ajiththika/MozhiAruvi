    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="mb-0 flex flex-col md:flex-row md:items-end md:justify-between gap-10">
        <div className="space-y-6">
           ...
           <div>
              <h1 className="text-3xl md:text-5xl font-black text-gray-800 tracking-tighter leading-none mb-4">Command Deck</h1>
              <p className="text-lg text-gray-500 font-medium leading-relaxed max-w-2xl italic opacity-80">
           ...
           {(['overview', 'plans', 'premium'] as const).map(tab => (
              <button
                 ...
                 className={cn(
                    ...
                    activeTab === tab ? "bg-white shadow-xl shadow-black/5 text-primary" : "text-gray-400 hover:text-gray-600"
                 )}
              >
                 {tab}
              </button>
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-3">
            <div className="lg:col-span-2 space-y-10">
              {/* Applications Table */}
              <div className="rounded-[2.5rem] border border-gray-100 bg-white shadow-2xl shadow-gray-200/20 overflow-hidden flex flex-col">
                <div className="flex items-center justify-between border-b border-gray-50 px-10 py-8">
                  <h3 className="text-xl font-black text-gray-800 tracking-tight">Mentor Applications</h3>
                  ...
                  <table className="w-full text-left border-collapse">
                    <tbody className="divide-y divide-gray-50">
                      {applications.length === 0 ? (
                        <tr><td className="py-20 text-center text-xs font-bold text-gray-300 uppercase tracking-widest">Queue Clear</td></tr>
                      ) : (
                        applications.slice(0, 4).map((app) => (
                          <tr key={app._id} className="hover:bg-gray-50/50 transition-all">
                            <td className="px-10 py-6 flex items-center gap-5">
                               ...
                               <div>
                                  <p className="text-sm font-black text-gray-800">{app.userId?.name || app.fullName}</p>
                                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{app.specialization}</p>
                               </div>
                            </td>
              ...
              <div className="rounded-[2.5rem] border border-gray-100 bg-white shadow-2xl shadow-gray-200/20 overflow-hidden flex flex-col">
                <div className="flex items-center justify-between border-b border-gray-50 px-10 py-8">
                  <h3 className="text-xl font-black text-gray-800 tracking-tight">Editorial Moderation</h3>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <tbody className="divide-y divide-gray-50">
                      {blogs.length === 0 ? (
                        <tr><td className="py-20 text-center text-xs font-bold text-gray-300 uppercase tracking-widest">Everything Published</td></tr>
                      ) : (
                        blogs.slice(0, 4).map((blog) => (
                          <tr key={blog._id} className="hover:bg-gray-50/50 transition-all">
                            <td className="px-10 py-6 flex items-center gap-5">
                               ...
                               <div>
                                  <p className="text-sm font-black text-gray-800">{blog.title}</p>
                                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">By {blog.author?.name}</p>
                               </div>
                            </td>
                            <td className="px-10 py-6 text-right">
            ...
            <div className="space-y-10">
              {/* Quick Actions */}
              <div className="rounded-[2.5rem] bg-gray-900 p-10 text-white shadow-2xl shadow-gray-900/40 relative overflow-hidden group">
                      ...
                      item.href ? (
                        <Link key={item.label} href={item.href} className="flex items-center justify-between p-5 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 transition-all group/btn">
                           <div className="flex items-center gap-4">
                              <item.icon className="h-5 w-5 text-gray-500 group-hover/btn:text-primary transition-colors" />
                              <span className="text-xs font-black uppercase tracking-widest text-gray-400 group-hover/btn:text-white transition-colors">{item.label}</span>
                           </div>
                           <ArrowRight className="h-4 w-4 text-gray-700 group-hover/btn:text-primary transition-all group-hover/btn:translate-x-1" />
                        </Link>
                      ) : (
      ...
      {activeTab === 'plans' && (
        <div className="animate-in slide-in-from-bottom-4 duration-700">
           <div className="rounded-[2.5rem] border border-gray-100 bg-white shadow-2xl shadow-gray-200/20 overflow-hidden">
              <div className="p-10 border-b border-gray-50">
                 <h3 className="text-2xl font-black text-gray-800 tracking-tight">Financial Models & Subscription Logic</h3>
                 <p className="text-sm font-medium text-gray-500 italic mt-2">Adjust pricing, limits and tier accessibility in real-time across the platform.</p>
              </div>
              <div className="overflow-x-auto">
                 <table className="w-full text-left border-collapse">
                    <thead className="bg-gray-50/50 border-b border-gray-100">
                       <tr>
                          <th className="px-10 py-6 text-[10px] font-black text-gray-500 uppercase tracking-widest">Plan Identity</th>
                          <th className="px-10 py-6 text-[10px] font-black text-gray-500 uppercase tracking-widest">Monthly / Yearly ($)</th>
                          <th className="px-10 py-6 text-[10px] font-black text-gray-500 uppercase tracking-widest">Limits (Cat/Tutor/Evt)</th>
                          <th className="px-10 py-6 text-[10px] font-black text-gray-500 uppercase tracking-widest">Stripe Integration</th>
                          <th className="px-10 py-6 text-[10px] font-black text-gray-500 uppercase tracking-widest text-right">Admin Control</th>
                       </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                       {planSettings.map(plan => {
                          ...
                          return (
                             <tr key={plan._id} className="hover:bg-gray-50/30 transition-all group">
                                <td className="px-10 py-6">
                                   <div className="flex items-center gap-5">
                                      <div className={cn(
                                         "h-12 w-12 rounded-2xl flex items-center justify-center font-black text-sm border",
                                         plan.plan === 'FREE' ? "bg-gray-50 text-gray-400 border-gray-100" : (plan.plan === 'PREMIUM' ? "bg-amber-50 text-amber-600 border-amber-100" : "bg-primary/5 text-primary border-primary/10")
                                      )}>
                                         {plan.plan.charAt(0)}
                                      </div>
                                      <span className="text-sm font-black text-gray-800 uppercase tracking-tight">{plan.plan.replace('_', ' ')}</span>
                                   </div>
                                </td>
                                <td className="px-10 py-6">
                                   {isEditing ? (
                                      ...
                                      <span className="text-xs font-black text-gray-600">${plan.monthlyPrice} / ${plan.yearlyPrice}</span>
                                   )}
                                </td>
                                <td className="px-10 py-6">
                                   {isEditing ? (
                                ...
                                <td className="px-10 py-6">
                                   {plan.plan === 'FREE' ? (
                                      <span className="text-[10px] font-bold text-gray-300 italic">No Bridge Required</span>
                                   ) : (
                                ...
                                <td className="px-10 py-6 text-right">
                                   {isEditing ? (
                                      <div className="flex justify-end gap-2">
                                         <button onClick={() => setEditingPlan(null)} className="h-10 w-10 flex items-center justify-center rounded-xl bg-gray-100 text-gray-400 hover:bg-gray-200 transition-colors">
                                      ...
                                      <button onClick={() => handleEditPlan(plan)} className="h-10 w-10 flex items-center justify-center rounded-xl bg-white border border-border/60 text-gray-400 hover:text-primary hover:border-primary/20 hover:shadow-lg transition-all">
          ...
          <div className="rounded-[3rem] border border-amber-100 bg-gradient-to-br from-amber-50/60 via-white to-orange-50/30 overflow-hidden shadow-2xl shadow-amber-200/10">
            <div className="flex items-center justify-between border-b border-amber-100 px-10 py-10">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h3 className="text-3xl font-black text-gray-800 tracking-tighter uppercase">Elite Collective</h3>
              ...
              <div className="flex gap-4">
                 <div className="hidden md:flex flex-col text-right">
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Active Revenue Nodes</span>
                    <span className="text-xl font-black text-gray-800 tracking-tight">${premiumUsers.reduce((acc, u) => acc + (u.subscription?.plan === 'PREMIUM' ? 7.94 : 3.81), 0).toFixed(2)} / mo</span>
                 </div>
              </div>
            </div>
                  ...
                  {premiumUsers.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-24 text-center">
                        <p className="text-xs font-black text-gray-300 uppercase tracking-[0.4em] italic">No active elite members</p>
                      </td>
                    </tr>
                  ) : (
                    premiumUsers.map((pu) => (
                      <tr key={pu._id} className="hover:bg-amber-50/40 transition-all group">
                        <td className="px-10 py-6">
                           <div className="flex items-center gap-5">
                              ...
                              <div>
                                 <p className="text-sm font-black text-gray-800 tracking-tight">{pu.name}</p>
                                 <p className="text-[10px] font-bold text-gray-400 italic mt-0.5">{pu.email}</p>
                              </div>
                           </div>
                        </td>
                        <td className="px-10 py-6">
                        ...
                        <td className="px-10 py-6">
                           <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{pu.subscription?.billingCycle}</span>
                        </td>
                        <td className="px-10 py-6">
                        ...
                        <td className="px-10 py-6">
                           <span className="text-[10px] font-black text-gray-400 font-serif italic">{pu.country || 'Global'}</span>
                        </td>
                        <td className="px-10 py-6 text-right">
