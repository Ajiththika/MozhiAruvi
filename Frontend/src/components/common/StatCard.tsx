...
export function StatCard({
  ...
  return (
    <div
      className={cn(
        "flex flex-col gap-4 rounded-[2rem] border border-gray-100 bg-white p-8 shadow-sm transition-all duration-300 hover:border-secondary/40 hover:-translate-y-1 hover:shadow-xl hover:shadow-secondary/5  dark:hover:border-secondary/40",
        className
      )}
    >
      ...
      <div className="space-y-1">
        ...
        <h3 className="text-3xl font-black tracking-tight text-gray-800 dark:text-white">
          {value}
        </h3>
        {description && (
          <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 leading-relaxed pt-1">
            {description}
          </p>
        )}
      </div>
